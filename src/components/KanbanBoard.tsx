                  ...
                  <div className="bg-white p-5 rounded-2xl border border-zinc-200/80 shadow-xs flex flex-col items-center justify-center flex-1 max-w-xs mx-auto md:mx-0">
                    ...
                    <div className="relative w-40 h-40 flex items-center justify-center">
                      <svg className="w-full h-full transform -rotate-90">
                        ...
                        return totalEverClosed > 0 ? Math.round((reactivados / totalEverClosed) * 100) : 0; })(), 100)) / 100} strokeLinecap="round" fill="transparent" />
                      </svg>
                      ...
                      const cerradosSinReactivar = localStatsLeads.filter(l => (l.estatus === 'CERRADO_VENTA' || l.estatus === 'CERRADO_ABANDONADO' || l.estatus === 'CERRADO') && !((l as any).reactivaciones > 0)).length;
                      const totalEverClosed = reactivados + cerradosSinReactivar;
                      return totalEverClosed > 0 ? Math.round((reactivados / totalEverClosed) * 100) : 0; })(), 100)}%</span><span className="text-[10px] text-zinc-400 font-bold uppercase">Reactivados</span></div>
                    </div>
                  </div>
                </div>
            ...
            {isAddingCol ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') handleAddColumn(); if (e.key === 'Escape') { setIsAddingCol(false); setNewColTitle(''); } }}
                  placeholder="Nombre de la fase..."
                  className="flex-1 px-3 py-2 text-sm border rounded-xl bg-zinc-50 focus:outline-none focus:ring-2 focus:ring-blue-300"
                  autoFocus
                />
                <button onClick={handleAddColumn} className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl">Agregar</button>
                <button onClick={() => { setIsAddingCol(false); setNewColTitle(''); }} className="px-4 py-2 border text-xs font-bold rounded-xl text-zinc-600">Cancelar</button>
              </div>
            ) : (
              <button
